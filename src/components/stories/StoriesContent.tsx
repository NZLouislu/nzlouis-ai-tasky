  ...
  const handleDocumentSave = useCallback(async (content: PartialBlock[]) => {
    ...
    try {
      if (isNewDocument) {
        ...
        if (!project.id || !document.documentType || !document.title) {
          throw new Error('Missing required fields: projectId, documentType, title');
        }
        ...
        if (result.isExisting) {
          updateDocument(activeDocumentId, { 
            content: result.document.content || markdownContent,
            lastSyncedAt: result.document.last_synced_at || new Date().toISOString(),
