...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if document of this type already exists for the project
    ...
    if (existingDoc) {
      return NextResponse.json(
        { error: `A ${documentType} document already exists for this project` },
        { status: 409 }
      );
    }

    // Generate document if content not provided
